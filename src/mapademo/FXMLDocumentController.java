...
public class FXMLDocumentController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        // CellFactory para el ListView de actividades
        actividades_listview.setCellFactory(listView -> new ListCell<Activity>() {
            @Override
            protected void updateItem(Activity act, boolean empty) {
                ...
                if (empty || act == null) {
                    ...
                    String fecha = act.getStartTime() != null
                            ? act.getStartTime().format(
                                    java.time.format.DateTimeFormatter.ofPattern("dd/MM/yyyy")
                            )
                            : "Sin fecha";
                    String distancia = String.format("%.1f km", act.getTotalDistance() / 1000.0);
                    setText("🏃 " + act.getName() + "  ·  " + fecha + "  ·  " + distancia);
                }
            }
        });
    ...
    private void onMapsButton(ActionEvent event) {
        try {
            ...
            FXMLMapaController controller = loader.getController();
            ...
            dato.nodeProperty().addListener((obs, oldNode, newNode) -> {
                if (newNode != null) {
                    newNode.setOnMouseEntered(e -> {
                        if (currentProjection == null || hoverMarker == null) {
                            return;
                        }
    ...
    private void cargarAnotacionesDeActividad(Activity activity) {
        ...
        for (Annotation ann : anotaciones) {
            if (ann.getGeoPoints() == null || ann.getGeoPoints().isEmpty()) {
                continue;
            }
